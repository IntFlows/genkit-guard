# Persistent decisions and fallback guard models - v0.1.0

Release 2 adds persistent audit delivery and explicit fallback for the local intent and PII models. It does not replace privacy-filter or introduce generative-model fallback.
...
Delivery order: console event (if enabled), store append, then onDecision callback. Store and callback run independently of console enabled/level settings. Both are awaited and fail closed. Failures are propagated, with no automatic retries or fallback to memory. A callback failure does not undo an already persisted record. Pre-execution decisions are not receipts proving execution; tool response scans happen after the tool has already run.
